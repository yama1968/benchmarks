...
#            'od_wait': 40,
    ...
    def preprocess_params(self, params):
        if self.learning_task == 'classification':
            params.update({'loss_function': 'Logloss', 'verbose': False, 'thread_count': 8, 'random_seed': 0})
        elif self.learning_task == 'regression':
            params.update({'loss_function': 'RMSE', 'verbose': False, 'thread_count': 8, 'random_seed': 0})
        return params
...
#            results = np.array([
#                np.array(map(lambda x: float(x.strip().split()[-1]), f.readlines()[1:])).min()] * 3) ##YY
            results = np.array(map(lambda x: float(x.strip().split()[-1]), f.readlines()[1:]) ##YY
